...
def index(request):
	...
	try:
		for city in yandex_city_list:
			yandex_weather.append(Weather.objects.filter(service=u'YX', city_id=city).order_by('-pk')[0])

		for city in yahoo_city_list:
			yahoo_weather.append(Weather.objects.filter(service=u'YO', city_id=city).order_by('-pk')[0])
	except IndexError:
